...
function Toggle({ checked, onChange, label, description }: { checked: boolean; onChange: () => void; label: string; description: string }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <button
        onClick={onChange}
        className={`relative w-10 h-6 rounded-full transition-colors ${checked ? "bg-sky-500" : "bg-slate-600"}`}
      >
        ...
        <div className="space-y-3">
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-700/50 border border-slate-600 text-white placeholder:text-slate-500 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500/50 transition-all"
            />
          </div>
          <div>
            <label className="flex items-center gap-1.5 text-sm text-slate-300 font-medium mb-1.5">
          ...
          <button
            ...
            className="flex items-center gap-2 bg-sky-500 hover:bg-sky-400 disabled:bg-sky-500/40 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-all"
          >
      ...
      <Section title="Notifications">
        <div className="space-y-4">
          ...
          <Toggle
            ...
            description="Receive alerts when your queue position changes"
            checked={notifications.queueUpdates}
            onChange={() => {
              setNotifications((n) => ({ ...n, queueUpdates: !n.queueUpdates }));
              toast.success("Preference saved");
            }}
          />
          <div className="border-t border-slate-700/50" />
          <Toggle
            label="Promotions & Health Tips"
            description="Occasional health tips and platform news"
            checked={notifications.promotions}
            onChange={() => {
              setNotifications((n) => ({ ...n, promotions: !n.promotions }));
              toast.success("Preference saved");
            }}
          />
        </div>
